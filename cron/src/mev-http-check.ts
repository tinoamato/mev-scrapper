...
import { MevHttpClient } from './mev-http-client';
import type { MevScrapeResult } from './mev-runner';

/**
 * Equivalente HTTP de checkExpedientes() (Selenium). Devuelve exactamente la misma
 * forma de resultado para que sea intercambiable sin tocar el resto del flujo.
 *
 * IMPORTANTE sobre la fecha: producción guarda la "hora de pared" tal como la
 * muestra MEV (14/08/2026 15:53:03 -> "2026-08-14T15:53:03"), que es lo que produce
 * el isoformat() del datetime naive de Python. Acá se replica esa convención a
 * propósito: cambiarla haría que los 35 expedientes aparezcan como "movidos" de golpe.
 * La zona horaria es un arreglo aparte, con migración de datos incluida.
 */

export type ExpedienteInput = { id: string; numeroExpediente: string; url: string | null };

export type ResultadoHttp = MevScrapeResult & {
